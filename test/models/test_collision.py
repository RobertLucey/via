...
from via.models.collisions.collision import Collision, Collisions
...
class CollisionTest(TestCase):
    ...
    def test_gps(self):
        collision = Collision(**self.TEST_COLLISION_DATA)

        self.assertEquals(
            collision.gps.serialize(),
            {"elevation": None, "lat": 52.44920547929465, "lng": -8.419577187763258},
        )
    ...
    def test_is_in_place(self):
        ...
        self.assertTrue(
            collision.is_in_place({"north": 100, "south": 0, "east": 10, "west": -10})
        )

        self.assertFalse(
            collision.is_in_place({"north": 10, "south": -10, "east": -10, "west": 10})
        )

    def test_parse(self):
        collision = Collision(**self.TEST_COLLISION_DATA)
        self.assertEqual(Collision.parse(collision), collision)
        self.assertEqual(
            Collision.parse(self.TEST_COLLISION_DATA).serialize(), collision.serialize()
        )
...
class CollisionsTest(TestCase):
    ...
    def test_danger(self):
        ...
        collisions.append(Collision(**self.TEST_COLLISION_DATA))
        collisions.append(Collision(**self.TEST_COLLISION_DATA))
