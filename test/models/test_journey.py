...
class JourneyTest(TestCase):
    ...
    def test_graph(self):
        ...
        graph_coordinates = []
        for frame_uuid, frame_data in journey.graph._node.items():
        ...
        data_no_time = [{'lat': d['lat'], 'lng': d['lng']} for d in data]

        self.assertEqual(graph_coordinates, data_no_time)

        self.assertEqual(
            list(journey.graph._node.keys()),
            [frame.uuid for frame in journey.frames]
        )
    ...
    def test_serialization(self):
        ...
        self.assertEqual(
            journey.serialize()['direct_distance'],
            3024.84802816048
        )
        self.assertEqual(
            journey.serialize()['duration'],
            770
        )
        self.assertEqual(
            journey.serialize()['indirect_distance'],
            {
                1: 3734.354622435669,
                5: 3734.354622435669,
        ...
        self.assertEqual(
            journey.serialize()['quality'],
            0.0
        )
        self.assertEqual(
            journey.serialize()['suspension'],
            True
        )
        self.assertEqual(
